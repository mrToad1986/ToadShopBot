from .config import KEYBOARD, AUTHOR, VERSION
...
# ответ при посещении блока с заказом
order = '''
<i>Название:</i> <b>{}</b>
...
# ответ по количеству заказов
order_number = '''
<b>Позиция в заказе № </b> <i>{}</i>
'''

# ответ при отсутствии заказов
no_orders = '''
<b>Заказ отсутствует!</b>
'''

# ответ при подтверждении оформления заказа
apply = '''
<b>Ваш заказ оформлен !!!</b>

<i>Общая стоимость заказа составляет:</i> <b>{} руб</b>

<i>Общее колическво позиций составляет:</i> <b>{} ед.</b>
